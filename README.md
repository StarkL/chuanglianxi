...
- **技术实现方案**：
  - **默认走端侧本地，守护核心语音隐私**：确立“本地优先（Local-First）”为默认策略。只要本地模型识别质量过关，语音转写**默认直接在本地设备离线完成**，音频数据永不离开手机/电脑，云端零上传；
  - **高精度与体验优先选型（SenseVoice-Small / Zipformer）**：选用阿里达摩院 SenseVoice-Small 或 Sherpa-ONNX Zipformer 量化小模型（字错率低至 2% 级，原生内置标点符号预测与智能断句，在红米 K30 等千元经典机型上亦能毫秒级出字）；
  - **首次下载，永久离线**：基于 WebAssembly 与 WebGPU 硬件加速，模型在用户首次使用时拉取并持久化缓存在本地 CacheStorage / IndexedDB 中；
  - **100% 全离线安全闭环**：音频录制 ➔ 端侧 ASR 转写 ➔ 自动标点排版 ➔ 结构化提炼 ➔ 本地 AES-256-GCM 强加密落盘，无网络也能随时随地流畅记录。
